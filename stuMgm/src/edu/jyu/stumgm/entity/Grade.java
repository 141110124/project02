...
+numDB+numDistributed+numTest;
	...
	//获取指定属性值
	public int getCertainSubject(String check){
		if(check.equals("electron")){
			return numElectron;
		}else if(check.equals("software")){
			return numSoftware;
		}else if(check.equals("security")){
			return numSecurity;
		}else if(check.equals("java")){
			return numJava;
		}else if(check.equals("db")){
			return numDB;
		}else if(check.equals("img")){
			return numImage;
		}else if(check.equals("distributed")){
			return numDistributed;
